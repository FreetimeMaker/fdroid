    ...
    for _, app := range appsList {
        ...
        for _, release := range releases {
            ...
            func() {
                ...
                appName := apps.GenerateReleaseFilename(app.Name(), tag)
                ...
                if _, err := os.Stat(appTargetPath); !errors.Is(err, os.ErrNotExist) {
                    log.Printf("Already have APK for version %q at %q", tag, appTargetPath)
                    return
                }

                log.Printf("Preparing Android build tools")
                if err := prepareAndroidBuildTools(); err != nil {
                ...
                if err := checkoutTag(repoPath, tag); err != nil {
                ...
                if err := copyFile(apkPath, appTargetPath); err != nil {
